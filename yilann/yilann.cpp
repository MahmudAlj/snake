...
////2. Creating the Food olmadi
////3. Creating the Snake tamam
////4. Moving the Snake tamam 
////5. Making the Snake eat the Food tamam
////6. Making the Snake grow longer tamam
////7. Checking for collision with edges 8 kail tamam
////8. Adding kitle 8 frame tamam
////9. Keeping score tamam
////10. Adding sounds yapmak ıstemedım
//
//
...
public:
    ...
    Elma(std::deque<Vector2> snakeBody) {
        Image image = LoadImage("path/to/your/image.png");  // Dosya yolunu kontrol edin
        if (image.width == 0) { // Resmin yüklenip yüklenmediğini kontrol edin
...
public:
    ...
    void CheckCollisionWithEdges() {
        if (snake.body[0].x >= noktsayimi || snake.body[0].x < 0) {
            GameOver();
        }
        if (snake.body[0].y >= noktsayimi || snake.body[0].y < 0) {
            GameOver();
        }
    }
